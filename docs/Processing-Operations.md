...
Chi tiết yêu cầu hiển thị số gửi/thực nhận, mã batch mới, vị trí và đội/trạng thái phân loại. Nút phân công chỉ hiện khi còn batch ở `AwaitingClassificationAssignment`. API danh sách, bộ lọc và chi tiết trả trạng thái hiệu lực `Completed` khi mọi output đang hoạt động đều có intake đang hoạt động với `ClassificationCompletedAt`; thiếu intake hoặc chưa phân loại xong vẫn là `ReturnReceived`. Trạng thái hiệu lực được tính khi đọc, không thay đổi mốc tổ chức hoàn tất tái chế và không cần sửa dữ liệu lịch sử. Nguồn được truy qua inventory → classified batch → classified items → intake → output → operation → inputs, không gán yêu cầu quyên góp giả cho đồ trả về.
